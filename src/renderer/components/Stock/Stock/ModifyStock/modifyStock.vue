      ...
      <nav class="level">
        ...
        <div class="level-left">
          <div class="level-item">
            <p class="subtitle is-4">Modify Stock</p>
          </div>
        </div>
            ...
            <td>
              <div class="select">
                <select v-model="row.unitName">
                  ...
                  <option class="Option" v-for="(unit, i) in units" :key="i">{{unit.main}}</option>
                </select>
              </div>
            </td>
            <td>
              <div class="select">
                <select v-model="row.stockGroup">
                  ...
                  <option class="Option" v-for="(group, i) in groups" :key="i">{{group.group}}</option>
                </select>
              </div>
            </td>
            <td>
              <p>
                ...
                <button class="button" @click="openChangeTaxCategoryModal(row)">Change</button>
              </p>
            </td>
            <td>
              <input
      ...
      <div class="modal-card">
        ...
        <section class="modal-card-body">
          <div>
            ...
            <input
              class="input"
              type="text"
              v-model="previousTaxDataForAUnit.taxCategory"
        ...
        <footer class="modal-card-foot">
          <button
            ...
            @click="changeTaxCategory(previousTaxDataForAUnit)"
          >Change</button>
      ...
      data.forEach((d, i) => {
        this.db.sales.find({ "detail.date": d.date }, (err, docs) => {
          if (err !== null) {
            ...
            // console.log(docs);
            this.updateSaleEnteryInFetechedData(
              docs,
              d.stockName,
              d.taxCategory,
              d.date
            ...
            if (i === data.length - 1) {
              ...
              updateRows.forEach(row => {
                console.log(row._id);
                this.db.sales.update(
                  { _id: "" + row._id },
                  { $set: { items: row.items, detail: row.detail } },
                  { multi: true, returnUpdatedDocs: true },
                  (err, affectedDocuments, numReplaced) => {
