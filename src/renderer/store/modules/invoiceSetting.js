...
const actions = {
    setInvoice ({ commit, state }, payload) {
        db.update({ _id: state._id }, { $set: {invoice: payload} }, {}, () => { commit('mutateInvoice', payload) })
    },
    incrementInvoice ({ commit, state }) {
        db.update({ _id: state._id }, { $set: {invoice: state.invoice + 1} }, {}, () => { commit('incrementInvoice') })
    },
    setInvoicePrefix ({ commit, state }, payload) {
        db.update({ _id: state._id }, { $set: {invoicePrefix: payload} }, {}, () => { commit('mutateInvoicePrefix', payload) })
    },
    setInvoiceSuffix ({ commit, state }, payload) {
        db.update({ _id: state._id }, { $set: {invoiceSuffix: payload} }, {}, () => { commit('mutateInvoiceSuffix', payload) })
    },
    setEnableInvoice ({ commit, state }, payload) {
        db.update({ _id: state._id }, { $set: {enableInvoice: payload} }, {}, () => { commit('mutateEnableInvoice', payload) })
    },
    setBillFormat ({ commit, state }, payload) {
        db.update({ _id: state._id }, { $set: {billFormat: payload} }, {}, () => { commit('mutateBillFormat', payload) })
    },
